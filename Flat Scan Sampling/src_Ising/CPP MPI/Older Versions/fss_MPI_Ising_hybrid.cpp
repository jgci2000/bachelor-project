...
const int num_threads_per_worker = 4;
...
int main(int argc, char **argv) 
{
    ...
    ll REP_per_thread = REP_per_worker / num_threads_per_worker;
    ...
    if (rank == root)
    {
        ...
        string console_output = "L: " + std::to_string(L) + " | REP: " + std::to_string(REP) + " | skip: " + std::to_string(skip) + " | workers: " + std::to_string(size - 1) + " (+1 root) | REP per worker " + std::to_string(REP_per_worker) + " | threads per worker: " + std::to_string(num_threads_per_worker) + " | REP per thread per worker : " + std::to_string(REP_per_thread);
    ...
    {   do
        {
            ...
            omp_set_num_threads(num_threads_per_worker);
            #pragma omp parallel reduction(+:accept_counter, reject_counter, k, k_saved, neo_previous[:NE*NE])//shared(accept_counter, reject_counter, k, k_saved, neo_previous)
            {
                ll hist_WL_thread[NE] = {0};
                ll hist_E_selected_thread[NE] = {0};
                // for (int i = 0; i < NE; i++)
                // {
                //     hist_WL[i] = 0;
                //     hist_E_selected[i] = 0;
                ...
                int ret[pos.size()];
                fss_scan_thread(ret, J, NE, pos, spins_WL, NN_table, NN, E_old, idx_E_old, energies);
                
                for (int i = 0; i < pos.size(); i++)
                {
                    // #pragma omp atomic
                        neo_previous[ret[i]] = neo_previous[ret[i]] + 1;
                }
                ...
                while (min_hist(hist_E_selected_thread, NE) < REP_per_thread)
                {
                    int idx_pos = xorshift64s(&state) % pos.size();
                    int flipped_pos = pos.at(idx_pos);
                    pos.erase(pos.begin() + idx_pos);
                    ...
                    if (ratio >= 1 || (ratio * 10000) > ((double) (xorshift64s(&state) % 10000)) || hist_WL_thread[idx_E_WL_new] == 0)
                    {
                        E_WL_old = E_WL_new;
                        ...
                        if (k >= k_saved + skip && hist_E_selected_thread[idx_E_WL_old] <= REP_per_worker || hist_WL_thread[idx_E_WL_new] == 0) 
                        {
                            hist_E_selected_thread[idx_E_WL_old]++;
                            k_saved = k;
                    ...
                    else
                    {
                        ...
                        if (k >= k_saved + skip && hist_E_selected_thread[idx_E_WL_old] <= REP_per_worker) 
                        {
                            hist_E_selected_thread[idx_E_WL_old]++;
                            k_saved = k;
