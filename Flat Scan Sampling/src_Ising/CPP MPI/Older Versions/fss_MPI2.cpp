...
int main(int argc, char **argv) 
{
    ...
    lli *neo_previous_root;
    lli accept_counter_root;
    lli reject_counter_root;
    lli k_root;
    lli k_saved_root;
    lli hits_root;
    lli sampled_ratio_root;
    ...
    if ((rank == root && REP % size != 0))
    {
        cout << "Make sure the number of worker processes might not be acceptable for this REP value (n_workers % REP != 0)." << endl;
        return 1;
    }

    lli REP_per_worker = REP / size;
    ...
    string console_output = "L: " + std::to_string(L) + " | REP: " + std::to_string(REP) + " | skip: " + std::to_string(skip) + " | workers: " + std::to_string(size) + " | REP per worker " + std::to_string(REP_per_worker);
    ...
    for (int q = 0; q < q_max - 1; q++) 
    {
        ...
        int sampled_ratio = REP_per_worker * hits * skip / k;

        MPI_Reduce(neo_previous, neo_previous_root, NE * NE, MPI_DOUBLE_PRECISION, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&accept_counter, &accept_counter_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&reject_counter, &reject_counter_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&k, &k_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&k_saved, &k_saved_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&sampled_ratio, &sampled_ratio_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
        MPI_Reduce(&hits, &hits_root, 1, MPI_LONG_LONG_INT, MPI_SUM, root, MPI_COMM_WORLD);
