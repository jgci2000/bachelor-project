...
int main(int argc, char **argv)
{
    ...
    if (rank == root)
    {
        ...
        string console_output = "run: " + std::to_string(run) + " | L: " + std::to_string(ising.L) + " | REP: " + std::to_string(REP) + " | skip: " + std::to_string(skip) + " | dim: " + std::to_string(ising.dim) + "D | lattie: " + ising.lattice + " | walkers: " + std::to_string(size - 1) + " | REP/walker: " + std::to_string(REP_worker);
