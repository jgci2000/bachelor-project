...
% END OF USER INPUT
%
wspace_filename = ['workspace_WL_sqrt_f_v1_L', int2str(L), '_q', int2str(q), '.mat'];
...
for f_counter = 1:length(f_temp)
    ...
    %
    WL_DOS_f_norm(hits, f_counter) = WL_log_DOS_f(hits, f_counter) - max(WL_log_DOS_f(hits, f_counter));
    WL_DOS_f_norm(hits, f_counter) = exp(WL_DOS_f_norm(hits, f_counter)) ./ sum(exp(WL_DOS_f_norm(hits, f_counter))) * norm_factor(q);
    %
end
%
WL_log_DOS_f_norm = zeros(length(E_list), length(f_temp));
WL_log_DOS_f_norm(WL_DOS_f_norm > 0) = log(WL_DOS_f_norm(WL_DOS_f_norm > 0));
%
