function [JDOS_aprox_frac, output] = function_flat_test_v48(L, q_max, skip, max_rw_steps, REP, BIG_K)
%
% clear
% close all
%
...
%
% wspace_filename = ['workspace_ft_v48_L', int2str(L), 'R1E', int2str(log10(REP)), '.mat'];
%
% disp([datestr(now,'dd/mm/yyyy HH:MM:SS'), ' | start run of 2D_SS_Ising_v48_L', int2str(L), '_R1E', int2str(log10(REP))]);
...
%
tEnd = toc(tStart);
disp([datestr(now,'dd-mm-yyyy HH:MM:SS'), ' finish ', int2str(BIG_K), ' | total run time ', num2str(tEnd), ' secs']);
%
