...
%
JDOS_aprox = load('./data/SS/JDOS_FSS_Ising_2D_SS_L16.txt');
%
L = 16;
%
T_max = 4.54; % maximum T value
nT = 50; % number of T values
%
...
E_list(:,1) = -1/2*N_atm*NN : 4 : 1/2*N_atm*NN;
%
...
%
% ENTROPY AND SPECIFIC HEAT FROM G DERIVATIVES
%
if length(T) > 1
    %
...
writematrix(E_list, ['./thermodynamic_variables/SS/L', int2str(L), '/energies.txt'])
writematrix(M_list, ['./thermodynamic_variables/SS/L', int2str(L), '/magnetizations.txt'])
writematrix(T, ['./thermodynamic_variables/SS/L', int2str(L), '/temperatures.txt'])
writematrix(M_minG', ['./thermodynamic_variables/SS/L', int2str(L), '/MminF.txt'])
