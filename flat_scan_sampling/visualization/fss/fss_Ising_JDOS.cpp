...
// Output location
#define SAVE_DIR(lattice, L)    "../data_JDOS/L" + to_string(L) + "_" + lattice + "/"

// Snapshot file
#define SNAPSHOT(snap) to_string(snap) 
...
int main(int argc, char **argv)
{
    ...
    if (L == 4)
    {
        int hist_q[16] = {1, 2, 3, 5, 6, 8, 9, 10, 9, 8, 6, 5, 3, 2, 1};

        for (int i = 0; i < 16; i++)
        {
            snapshot_q[i] = snapshot * hist_q[i];
        }
    }
    ...
    for (int q = 1; q <= q_max; q++)
    {
        ...
        for (int flip_idx = 0; flip_idx < flip_list[0].size(); flip_idx++)
        {
            ...
            JDOS[idx_E_tmp * NM + q + 1] += JDOS[idx_E_config * NM + q] / REP;
        }
        ...
        while (min_hist(hist_E_selected, NE) < REP)
        {
            ...
            if (hist_E_selected[idx_E_config] < REP && k % skip == 0 || hist_E_selected[new_idx_E_config] == 0)
            {
                for (int flip_idx = 0; flip_idx < flip_list[0].size(); flip_idx++)
                {
                    int delta_E = 0;
            ...
            if (k % snapshot_q[q] == 0)
            {
                ...
                for (int j = 0; j < NM; j++)
                {
                    ...
                    for (int i = 0; i < NE; i++)
                        if (JDOS[i * NM + j] > 0)
                            sum_JDOS += JDOS[i * NM + j];

                    for (int i = 0; i < NE; i++)
                        JDOS_norm[i * NM + j] = JDOS[i * NM + j] * norm_factor[j] / sum_JDOS;
                }
                
                std::ofstream file1((string) SAVE_DIR(lattice, L) + SNAPSHOT(snapshot_idx) + "_JDOS.txt");
                for (int i = 0; i < NE; i++)
                {
                    for (int j = 0; j < NM; j++) 
                        file1 << JDOS_norm[i * NM + j] << " ";
                    file1 << "\n";
                }
                file1.close();
        ...
        cout << console_output << endl;
        cout << "row steps: " << k << "; snapshots tanken: " << snapshot_idx << endl; 
    }
