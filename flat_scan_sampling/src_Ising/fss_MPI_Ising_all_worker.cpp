...
int main(int argc, char **argv)
{
    ...
    for (int q = 1; q <= q_max; q++)
    {
        ...
        // Shuffle configuration for less correlation between walkers

        shuffle(JDOS, pow(10, 0), flip_list, spins_vector, q, N_atm, NN, NM, NN_table, E_config, idx_E_config, energies);
        ...
        while (min_hist(hist_E_selected, NE) < REP_worker)
        {
            ...
            if (hist_E_selected[idx_E_config] < REP_worker && k % skip == 0)
            {
                for (int flip_idx = 0; flip_idx < flip_list[0].size(); flip_idx++)
                {
                    int delta_E = 0;
        ...
        if (rank == root)
        {
            for (int j = 0; j <= q; j++)
                for (int i = 0; i < NE; i++)
                    JDOS_root[i * NM + j] = JDOS_root[i * NM + j] / size;
