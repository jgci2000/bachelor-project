...
int main(int argc, char **argv) 
{
    ...
    ll *neo_previous_root = new ll[NE * NE];
    ...
    for (int q = 0; q < q_max - 1; q++) 
    {
        ...
        MPI_Bcast(JDOS_frac, NE * NM, MPI_DOUBLE, root, MPI_COMM_WORLD);
        ...
        ll k_saved_root = 0;
        int hits_root = 0;
        int sampled_ratio_root = 0;
        
        MPI_Reduce(neo_previous_root, neo_previous, NE * NE, MPI_LONG_LONG, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&accept_counter_root, &accept_counter, 1, MPI_LONG_LONG, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&reject_counter_root, &reject_counter, 1, MPI_LONG_LONG, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&k_root, &k, 1, MPI_LONG_LONG, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&k_saved_root, &k_saved, 1, MPI_LONG_LONG, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&hits_root, &hits, 1, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
        MPI_Reduce(&sampled_ratio_root, &sampled_ratio, 1, MPI_INT, MPI_SUM, root, MPI_COMM_WORLD);
        cout << "a" << endl;
    ...
    if (rank == root)
    {
        ...
        delete[] JDOS, norm_factor;
    }
    

    delete[] JDOS_frac, neo_previous_root;
