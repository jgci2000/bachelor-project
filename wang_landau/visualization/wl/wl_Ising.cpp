...
int main(int argc, char **argv)
{
    ...
    while(f > f_final)
    {
        ...
        if (mc_sweep_tot % snapshot == 0)
        {
            for (int q = 0; q < NM; q++)
            {
                ...
                ld sum_ln_JDOS = ln_JDOS[first_idx * NM + q];
                if (temp != 0)
                    sum_ln_JDOS = ln_JDOS[first_idx * NM + q] + log(temp);
    ...
    // std::ofstream file1((string) SAVE_DIR(lattice, L) + save_file + ".txt");
    // for (int i = 0; i < NE; i++) 
    // {
    //     for (int j = 0; j < NM; j++) 
    //         file1 << JDOS[i * NM + j] << " ";
    ...
    // std::ofstream file2((string) SAVE_DIR(lattice, L) + save_file + "_data.txt");
    ...
    // std::ofstream file3((string) SAVE_DIR(lattice, L) + save_file + "_console_logs.txt");
