...
// and uses the 1/t modification
// introduced by Belardinelli in 2007
//
...
int main(int argc, char **argv)
{
    ...
    // Do a trial run to know the energies
    ...
    } while (min_hist(hist, NE * NM) < average_hist(hist, NE * NM) * flatness);

    vector<int> nnz_hist; 
    for (int i = 0; i < NE * NM; i++)
        if (hist[i] > 0)
            nnz_hist.push_back(i);
    ...
    string console_output = "L: " + to_string(L) + " | f_final: 1+1E" + to_string((int) log10(f_final - 1)) + 
    ...
    while(f > f_final)
    {
        ...
        if (mc_sweep % 1000 == 0) 
        {
            ...
            for (int i = 0; i < nnz_hist.size(); i++)
                if (hist[nnz_hist.at(i)] == 0)
                    all_visited = false;
    ...
    fs::create_directories(SAVE_DIR(lattice, L, (int) f_final_exp));

    std::ofstream file1((string) SAVE_DIR(lattice, L, (int) f_final_exp) + save_file + ".txt");
    for (int i = 0; i < NE; i++) 
    {
        for (int j = 0; j < NM; j++) 
            file1 << JDOS[i * NM + j] << " ";
    ...
    std::ofstream file2((string) SAVE_DIR(lattice, L, (int) f_final_exp) + save_file + "_data.txt");
    ...
    std::ofstream file3((string) SAVE_DIR(lattice, L, (int) f_final_exp) + save_file + "_console_logs.txt");
